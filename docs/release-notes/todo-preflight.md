...
TODO CLI dry-runs and dashboard previews now validate the complete Captain run input before execution. Capability warnings appear with the preview; invalid attachments, judge declarations, and runtime policy fail before a run is admitted. Dispatch repeats the same checks against its actual provider and configuration.
