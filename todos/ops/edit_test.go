...
func TestBuildTodoEdit(t *testing.T) {
	t.Run("splits content and state flags", func(t *testing.T) {
		changes, err := BuildEdit(EditFlags{
	...
	t.Run("status alone is a valid edit", func(t *testing.T) {
		changes, err := BuildEdit(EditFlags{Status: "draft"})
		if err != nil {
			t.Fatalf("BuildEdit: %v", err)
		}
	...
	t.Run("rejects a projected status", func(t *testing.T) {
		_, err := BuildEdit(EditFlags{Status: "failed"})
		if err == nil {
			t.Fatal("BuildEdit(--status failed) = nil error, want rejection")
		}
	...
	t.Run("rejects an unknown priority", func(t *testing.T) {
		if _, err := BuildEdit(EditFlags{Priority: "critical"}); err == nil {
			t.Fatal("BuildEdit(--priority critical) = nil error, want rejection")
		}
	})

	t.Run("rejects an empty title", func(t *testing.T) {
		if _, err := BuildEdit(EditFlags{Title: ptr("   ")}); err == nil {
			t.Fatal("BuildEdit(--title '   ') = nil error, want rejection")
		}
	})
	...
	t.Run("rejects a no-op edit", func(t *testing.T) {
		...
		if err == nil {
			t.Fatal("BuildEdit() = nil error, want rejection")
		}
	...
	t.Run("rejects an empty plan", func(t *testing.T) {
		if _, err := BuildEdit(EditFlags{Plan: ptr("   ")}); err == nil {
			t.Fatal("BuildEdit(--plan '   ') = nil error, want rejection")
		}
	})
}
