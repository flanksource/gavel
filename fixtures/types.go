...
type FixtureTest struct {
	...
	// Measurements overrides file-level measurement extraction for command blocks.
...
type FrontMatter struct {
	...
	// Measurements are CEL expressions extracted independently from each sample.
	Measurements []MeasurementSpec `yaml:"measurements,omitempty" json:"measurements,omitempty"`
...
func (f *FrontMatter) CleanMetadata() {
	...
	delete(f.Metadata, "measurements")
...
// OutcomeStatus separates the command, assertion, and measurement verdicts that
...
// command or measurement evidence from the same process execution.
...
// MeasurementSample records the extraction result for one configured measurement.
...
type FixtureSample struct {
	...
	Measurements map[string]MeasurementSample `json:"measurements,omitempty"`
}
...
type FixtureOutcomes struct {
	...
	Measurements *FixtureOutcome `json:"measurements,omitempty"`
}

// MeasurementComparison records the finalized comparison to another logical row.
...
// MeasurementSummary contains valid raw samples, the selected aggregate, and any
// absolute or baseline threshold verdict for one measurement.
type MeasurementSummary struct {
	...
	Value      *float64               `json:"value,omitempty"`
	Threshold  *MeasurementThreshold  `json:"threshold,omitempty"`
	Comparison *MeasurementComparison `json:"comparison,omitempty"`
	Status     OutcomeStatus          `json:"status"`
	Error      string                 `json:"error,omitempty"`
}
...
func (f FixtureResult) PrettyFull() api.Textable {
	...
	} else if f.Outcomes != nil && f.Error != "" {
	...
	if len(f.Measurements) > 0 {
		names := make([]string, 0, len(f.Measurements))
		for name := range f.Measurements {
			names = append(names, name)
		}
		sort.Strings(names)
		for _, name := range names {
			t = t.NewLine().Append(formatMeasurementSummary(name, f.Measurements[name]), "font-mono text-xs")
		}
	}
...
func formatMeasurementSummary(name string, measurement MeasurementSummary) string {
	value := "incomplete"
	if measurement.Value != nil {
		value = fmt.Sprintf("%.6g %s", *measurement.Value, measurement.Unit)
	}
	text := fmt.Sprintf("measurement %s: %s (%s, %s)", name, value, measurement.Aggregate, measurement.Status)
	if measurement.Comparison != nil {
		text += fmt.Sprintf(", baseline %s %.6g %s, regression %.2f%%", measurement.Comparison.Baseline, measurement.Comparison.BaselineValue, measurement.Unit, measurement.Comparison.RegressionPercent)
	}
	return text
}
...
func formatSampleSummaries(samples []FixtureSample) string {
	...
	for _, sample := range samples {
		...
		if len(sample.Measurements) > 0 {
			measurementStatus := OutcomePASS
			for _, measurement := range sample.Measurements {
				if measurement.Status == OutcomeERR {
					measurementStatus = OutcomeERR
					break
				}
				if measurement.Status == OutcomeNotEvaluated {
					measurementStatus = OutcomeNotEvaluated
				}
			}
			line += fmt.Sprintf(" measurements=%s", measurementStatus)
		}
