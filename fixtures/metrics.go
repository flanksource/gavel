...
func validateFixtureConfiguration(fixtures []FixtureTest) error {
	...
	for _, fixture := range fixtures {
		for _, metric := range fixture.metricSpecs() {
			...
			if metric.Baseline == fixture.Name {
				if len(fixture.Metrics) > 0 {
					return fmt.Errorf("fixture %q metric %q: baseline must name another row", fixture.Name, metric.Name)
				}
				continue
			}
...
func finalizeMetricComparisons(results []*FixtureResult) {
	...
	for _, result := range results {
		for _, metric := range result.Test.metricSpecs() {
			...
			if metric.Threshold != nil && metric.Threshold.RegressionPercent != nil {
				threshold := *metric.Threshold.RegressionPercent
