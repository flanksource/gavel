...
func TestCommitMessageSchema_TypeIsEnumerated(t *testing.T) {
	_, schemaJSON, err := renderCommitPrompt(sampleCommit(), commitMessagePrompt, commitPromptData(0, defaultTypes()))
...
func TestCommitMessageSchema_EnumFollowsConfiguredTypes(t *testing.T) {
	...
	promptText, schemaJSON, err := renderCommitPrompt(sampleCommit(), commitMessagePrompt, commitPromptData(0, allowed))
	require.NoError(t, err)
