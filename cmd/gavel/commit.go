...
type CommitOptions struct {
	...
	IgnoreCheck     string `flag:"ignore-check" help:"Behavior when a staged file matches commit.gitignore: prompt|fail|skip" default:"prompt"`
	LinkedDepsCheck string `flag:"linked-deps-check" help:"Behavior when a staged go.mod/package.json references a path outside the git root: prompt|fail|skip" default:"prompt"`
...
package.json file:/link:/portal: or ../ paths). Each violation prompts to
(1) unstage the manifest so the bad edit is dropped from the commit, or
(2) cancel. --linked-deps-check=fail|skip overrides the prompt.
...
Examples:
  ...
  gavel commit --ignore-check=fail      # error on any commit.gitignore match`
}
...
func runCommit(opts CommitOptions) (any, error) {
	...
	linkedDepsMode := opts.LinkedDepsCheck
	if linkedDepsMode == "" {
		linkedDepsMode = cfg.Commit.LinkedDeps.Mode
	}
	...
	result, err := commitpkg.Run(context.Background(), commitpkg.Options{
		...
		LinkedDepsCheck: linkedDepsMode,
		Config:          cfg.Commit,
	})
