...
export function JsonView({ data, name, depth = 0 }: Props) {
  ...
  if (entries.length === 0) {
    return <span className="text-gray-400">{bracket[0]}{bracket[1]}</span>;
  }

  return (
    <div className="text-sm font-mono" style={{ paddingLeft: depth > 0 ? '12px' : '0' }}>
      <span
        className="cursor-pointer hover:bg-gray-100 rounded px-0.5 select-none"
        onClick={() => setOpen(!open)}
      >
        <span className="text-gray-400 text-xs mr-1">{open ? '▼' : '▶'}</span>
        {name && <span className="text-purple-600">{name}</span>}
        {name && <span className="text-gray-400">: </span>}
        {!open && <span className="text-gray-400">{bracket[0]} {entries.length} {isArray ? 'items' : 'keys'} {bracket[1]}</span>}
        {open && <span className="text-gray-400">{bracket[0]}</span>}
      </span>
      {open && (
        <>
          {entries.map(([key, val]) => (
            <div key={key} className="pl-3 border-l border-gray-200 ml-1">
              {typeof val === 'object' && val !== null ? (
                <JsonView data={val} name={String(key)} depth={depth + 1} />
              ) : (
                <div>
                  <span className="text-purple-600">{isArray ? '' : String(key)}</span>
