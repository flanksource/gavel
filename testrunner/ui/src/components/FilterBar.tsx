...
interface Props {
  ...
  counts: { passed: number; failed: number; warned: number; skipped: number; pending: number; running: number; timedout: number };
  frameworks: string[];
}
...
const STATUS_DEFS: { key: string; label: string; badge: string; activeBg: string; activeBorder: string }[] = [
  ...
  { key: 'warned', label: 'Warned', badge: 'bg-amber-400', activeBg: 'bg-amber-50', activeBorder: 'border-amber-300' },
