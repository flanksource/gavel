...
type KubernetesRef struct {
	...
	// JSONPath to the field within the resource
	JSONPath string `json:"path,omitempty"`
	// File location within multi-document YAML
	StartLine int `json:"start_line,omitempty"`
	EndLine   int `json:"end_line,omitempty"`
	// Resource identifiers for filtering
...
// ChangeSeverity indicates the potential impact of a change
// Note: This is kept separate from models.Severity to avoid circular imports
// but uses the same string values for compatibility
...
// ExtendedPatch wraps jsonpatch.JsonPatchOperation with additional old value tracking
...
// KubernetesChange represents a change to a Kubernetes resource in a commit
type KubernetesChange struct {
	...
	Before           map[string]any `json:"before,omitempty" pretty:"hide"`
	After            map[string]any `json:"after,omitempty" pretty:"hide"`
...
type VersionChange struct {
	...
	ValueType  string            `json:"value_type,omitempty"` // "semver", "sha256", "git-sha", "combined"
	Digest     string            `json:"digest,omitempty"`     // For SHA256 digests when tracking separately
}

// AnalyzeVersionChange parses semantic versions and determines the type of version change
func AnalyzeVersionChange(oldVer, newVer string) VersionChange {
	...
	// Handle Docker image tags that may have 'v' prefix
	...
	if oldErr != nil || newErr != nil {
		// Not valid semver, leave as unknown
		return vc
	}

	// Compare versions to determine change type (only for upgrades)
...
func (vc VersionChange) Pretty() api.Text {
	t := utils.HumanDiff(vc.OldVersion, vc.NewVersion)

	// Add change type indicator with icon
...
// Pretty returns a formatted text representation of the Kubernetes change
func (kc KubernetesChange) Pretty() api.Text {
	// Start with severity badge if present
	t := api.Text{}
	if kc.Severity != "" {
	...
	if len(kc.Before) > 0 && len(kc.After) > 0 {
		...
		// Generate summary using DiffMap
		beforeMap := utils.DiffMap[any](kc.Before)
		afterMap := utils.DiffMap[any](kc.After)
...
// getSeverityBadge returns the badge text and color style for a severity level
...
// YAMLDocument represents a parsed YAML document with its boundaries
