...
// The config tabs. `status` and `test` are folded into AI Defaults and Tests so
// every config section is reachable (they have no tab of their own but must not
// be silently uneditable).
export const TABS: TabDef[] = [
  { id: "ai", label: "AI Defaults", sections: ["ai", "status"] },
  { id: "commit", label: "Commit", sections: ["commit"] },
  { id: "pr", label: "Pull Requests", sections: ["pr"] },
...
export function promptIdOf(
  node: JsonSchemaProperty | undefined
): string | undefined {
  const id = node?.["x-prompt-id"];
  return typeof id === "string" ? id : undefined;
}

export function usesPromptPicker(
  node: JsonSchemaProperty | undefined
): boolean {
  return node?.["x-prompt-picker"] === true;
}
...
function decorateNode(node: JsonSchemaProperty, topLevel: boolean): void {
  ...
  if (node.properties) {
    for (const [key, child] of Object.entries(node.properties)) {
      if (child.title == null) {
        child.title =
          (topLevel ? SECTION_TITLES[key] : undefined) ??
          FIELD_TITLES[key] ??
          humanizeKey(key);
      }
  ...
  for (const key of ["oneOf", "anyOf", "allOf"] as const) {
    ...
    if (Array.isArray(subs))
      for (const sub of subs) decorateNode(sub as JsonSchemaProperty, false);
  }
}
...
function stampPaths(
  node: JsonSchemaProperty | JsonSchemaObject,
  prefix: string
): void {
