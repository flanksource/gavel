...
function useRunPromptPreview(dir: string, ref: string, options: TodoRunOptions) {
  ...
  }, [dir, ref, options.driver, options.backend, options.model, options.effort]);
...
export function TodoSessionStart({
  ...
  const context = useTodoRunContext();
  const options = useMemo(() => loadLastTodoRunOptions('run', context), [context]);
  const presentation = todoRunButtonPresentation(options, context);
  const runtime = todoRunModeLabel(options, context);
    ...
    <div className="m-3 flex min-h-0 flex-1 flex-col items-center overflow-y-auto rounded-md border border-border bg-card px-6 py-8 text-center">
      ...
      {onRun && (
        <div className="mt-5 flex items-center justify-center gap-2">
          <TodoRunActionButton action="run" disabled={runDisabled} loading={runBusy} onRun={onRun} onAdvanced={onAdvanced ?? (() => {})} />
          <TodoRunActionButton action="plan" disabled={runDisabled} loading={runBusy} onRun={onRun} onAdvanced={onAdvanced ?? (() => {})} />
        </div>
      )}
    </div>
  );
