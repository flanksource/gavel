...
function ProcessFavicon({ project, port }: { project: string; port?: number }) {
  ...
  return <img src={favicon} alt="" className="h-4 w-4 shrink-0 rounded-sm" onError={() => setFaviconFailed(true)} />;
}

export function ProcessPortLink({ port }: { project: string; port: number }) {
  const href = `http://localhost:${port}`;
  return (
    <a href={href} target="_blank" rel="noreferrer" title={`Open localhost:${port}`} className="inline-flex items-center gap-0.5 text-[10px] tabular-nums text-blue-500 hover:underline mr-1" onClick={(e) => e.stopPropagation()}>
      :{port}
    </a>
  );
}
    ...
    <>
      <tr className="border-b border-gray-100 hover:bg-gray-50">
        ...
        <td className="px-2 text-right tabular-nums">
          <CpuBars metricKey={runKey(project.name, proc)} />
        </td>
        <td className="px-2 text-right tabular-nums">
          <MemoryBars metricKey={runKey(project.name, proc)} />
        </td>
        <td className="px-2 text-right tabular-nums">{filesLabel(proc)}</td>
        <td className="px-2">
          {ports.map((port) => (
            <ProcessPortLink key={port} project={project.name} port={port} />
          ))}
        </td>
...
export function ProcessTable({ procs, onChanged, showWorkspace = true }: { procs: FlatProc[]; onChanged: () => void; showWorkspace?: boolean }) {
  ...
  const sorted = [...procs].sort((a, b) => activeFirst(a.proc.status) - activeFirst(b.proc.status) || a.proc.name.localeCompare(b.proc.name));
  return (
        ...
        <tr className="text-[10px] uppercase tracking-wide text-gray-400 border-b border-gray-200">
          ...
          <th className="px-2 text-right font-medium">
            <span className="inline-flex items-center justify-end gap-1">
              <UiActivity size={12} aria-hidden /> CPU
            </span>
          </th>
          <th className="px-2 text-right font-medium">
            <span className="inline-flex items-center justify-end gap-1">
              <UiDatabase size={12} aria-hidden /> Mem
            </span>
          </th>
