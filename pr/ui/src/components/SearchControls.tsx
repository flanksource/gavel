...
export function SearchControls({ config, onChange }: Props) {
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      >
        <iconify-icon icon="codicon:person" className="mr-0.5 text-[10px]" />
        @me
      </button>
      <button
        className={`text-xs px-2 py-0.5 rounded-full border transition-colors ${
      ...
      >
        <iconify-icon icon="codicon:organization" className="mr-0.5 text-[10px]" />
        All
      </button>
      ...
      >
        <iconify-icon icon="codicon:hubot" className="mr-0.5 text-[10px]" />
        Bots
      </button>
    </div>
  );
